...
# print(model.summary())
#
# dot_img_file = './custom_unet_generator_v2.png'
# 
# import tensorflow as tf
# tf.keras.utils.plot_model(model, to_file=dot_img_file,dpi=400, show_shapes=True)
...
def custom_unet_descriminator_v2(target=True):
    ...
    conv6 = tf.keras.layers.Conv2D(
        1, 4, strides=1, kernel_initializer='he_normal')(conv5)
...
# dot_img_file = './custom_unet_descriminator_v2.png'
#
# import tensorflow as tf
# tf.keras.utils.plot_model(model, to_file=dot_img_file,dpi=400, show_shapes=True)

def custom_unet_descriminator_dilated_v2(target=True):
    inp = tf.keras.layers.Input(shape=[None, None, 3], name='input_image') #shape=[None, None, 3] ,[256, 256, 3]
    x = inp
    if target:
        tar = tf.keras.layers.Input(shape=[None, None, 3], name='target_image') #shape=[None, None, 3] , [256, 256, 3]
        x = tf.keras.layers.concatenate([inp, tar])
...
def custom_unet_generator_v3(input_size=(256, 256, 3)):

    inputs = keras.layers.Input(input_size)
    conv1 = keras.layers.Conv2D(40, 3, activation=tf.keras.layers.LeakyReLU(), padding='same', kernel_initializer='he_normal',use_bias=False)(inputs)
    conv1 = keras.layers.Conv2D(40, 3, activation=tf.keras.layers.LeakyReLU(), padding='same', kernel_initializer='he_normal',use_bias=False,strides=2)(conv1)

    conv2 = keras.layers.Conv2D(80, 3, activation=tf.keras.layers.LeakyReLU(), padding='same', kernel_initializer='he_normal',use_bias=False)(conv1)
    conv2 = keras.layers.Conv2D(80, 3, activation=tf.keras.layers.LeakyReLU(), padding='same', kernel_initializer='he_normal',use_bias=False,strides=2)(conv2)
    conv2 = InstanceNormalization()(conv2)

    conv3 = keras.layers.Conv2D(160, 3, activation=tf.keras.layers.LeakyReLU(), padding='same', kernel_initializer='he_normal',use_bias=False)(conv2)
    conv3 = keras.layers.Conv2D(160, 3, activation=tf.keras.layers.LeakyReLU(), padding='same', kernel_initializer='he_normal',use_bias=False,strides=2)(conv3)
    conv3 = InstanceNormalization()(conv3)

    conv4 = keras.layers.Conv2D(320, 3, activation=tf.keras.layers.LeakyReLU(), padding='same', kernel_initializer='he_normal',use_bias=False)(conv3)
    conv4 = keras.layers.Conv2D(320, 3, activation=tf.keras.layers.LeakyReLU(), padding='same', kernel_initializer='he_normal',use_bias=False,strides=2)(conv4)
    conv4 = InstanceNormalization()(conv4)

    conv5 = keras.layers.Conv2D(640, 3, activation=tf.keras.layers.LeakyReLU(), padding='same', kernel_initializer='he_normal',use_bias=False,strides=2)(conv4)
    conv5 = keras.layers.Conv2D(640, 3, activation=tf.keras.layers.LeakyReLU(), padding='same', kernel_initializer='he_normal',use_bias=False)(conv5)


    up6 = keras.layers.Conv2DTranspose(320, 2, activation=tf.keras.layers.ReLU(), padding='same', kernel_initializer='he_normal',use_bias=False,strides=2)(conv5)
    merge6 = keras.layers.concatenate([conv4, up6], axis=3)
    conv6 = keras.layers.Conv2D(320, 3, activation=tf.keras.layers.ReLU(), padding='same', kernel_initializer='he_normal',use_bias=False)(merge6)
    conv6 = Conv2D(512, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv6)
    conv6 = Dropout(0.5)(conv6)

    up7 = keras.layers.Conv2DTranspose(160, 2, activation=tf.keras.layers.ReLU(), padding='same', kernel_initializer='he_normal',use_bias=False,strides=2)(conv6)
    merge7 = keras.layers.concatenate([conv3, up7], axis=3)
    conv7 = keras.layers.Conv2D(160, 3, activation=tf.keras.layers.ReLU(), padding='same', kernel_initializer='he_normal',use_bias=False)(merge7)
    conv7 = keras.layers.Conv2D(160, 3, activation=tf.keras.layers.ReLU(), padding='same', kernel_initializer='he_normal',use_bias=False)(conv7)
    conv7 = Dropout(0.5)(conv7)

    up8 = keras.layers.Conv2DTranspose(80, 2, activation=tf.keras.layers.ReLU(), padding='same', kernel_initializer='he_normal',use_bias=False,strides=2)(conv7)
    merge8 = keras.layers.concatenate([conv2, up8], axis=3)
    conv8 = keras.layers.Conv2D(80, 3, activation=tf.keras.layers.ReLU(), padding='same', kernel_initializer='he_normal',use_bias=False)(merge8)
    conv8 = keras.layers.Conv2D(80, 3, activation=tf.keras.layers.ReLU(), padding='same', kernel_initializer='he_normal',use_bias=False)(conv8)


    up9 = keras.layers.Conv2DTranspose(40, 2, activation=tf.keras.layers.ReLU(), padding='same', kernel_initializer='he_normal',use_bias=False,strides=2)(conv8)
    merge9 = keras.layers.concatenate([conv1, up9], axis=3)
    conv9 = keras.layers.Conv2D(40, 3, activation=tf.keras.layers.ReLU(), padding='same', kernel_initializer='he_normal',use_bias=False)(merge9)
    conv9 = keras.layers.Conv2D(40, 3, activation=tf.keras.layers.ReLU(), padding='same', kernel_initializer='he_normal',use_bias=False)(conv9)
    conv9 = tf.keras.layers.Conv2DTranspose(3, 3, strides=2,padding='same', kernel_initializer='he_normal',activation='tanh')(conv9)  # (bs, 256, 256, 3)
...
# model = custom_unet_generator_v3()
# print(model.summary())
